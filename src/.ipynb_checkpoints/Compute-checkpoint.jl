...
function _analyze_real_world_ternary_single_asset(R::Array{Float64,1}, Δt::Float64, ϵ::Float64)::MyRealWorldTernarySharePriceTreeParameters
    ...
    parameters = MyRealWorldTernarySharePriceTreeParameters();
    parameters.p = p;
    parameters.p̄ = p̄;
    parameters.q = q;
    parameters.u = mean(u);
...
Var(model::MyAdjacencyBasedTrinomialSharePriceTree, levels::Array{Int64,1}; 
    startindex::Int64 = 0) = _𝕍(model, levels, startindex = startindex)

expectation(model::MyAdjacencyBasedTrinomialSharePriceTree, levels::Array{Int64,1}; 
...
function populate(model::MyAdjacencyBasedTrinomialSharePriceTreeModel; 
    ...
    P[0] = Sₒ; # set the price at the root of the tree
